...
class Search
{
	...
	function doGet($route){
		...
		// SELECT id FROM employees WHERE first_name LIKE '%sur%' OR surname LIKE '%sur%'
		// name LIKE '%муфта%' OR name LIKE '%200%'
		$search = str_replace(" ","|",$str);
		// $sql = 'SELECT * FROM clients WHERE name RLIKE "'.$search.'" OR sname RLIKE "'.$search.'"';
		$sql = "SELECT * FROM client1 WHERE  descriptions LIKE '%{$this->descriptions}%' OR location LIKE '%{$this->descriptions}%' OR city LIKE '%{$this->descriptions}%'  ";
