...
class Get_favorites
{
	...
	function doGet($route){
		...
		$sql = 'SELECT * FROM client1 INNER JOIN favorites ON client1.id = favorites.ad_id AND client1.status = 1 AND favorites.user_id=:user_id';
	    ...
	    if($stmt->rowCount() > 0)
	    {
			...
			return $arr_json = array('status' => 200, 'favorites' => $row);
	    }else{
	...
	function doPost(){
		//POST method
		$sql = 'INSERT birimdik.users (login,password,avatar,numbers,local) VALUES (:login,:password,:avatar,:numbers,:local)';
	    $stmt = $this->db->prepare($sql);
