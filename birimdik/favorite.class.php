...
class Favorite
{
	...
	function doGet($route){
		//GET method
		$sql = 'SELECT * FROM favorites WHERE ad_id = :ad_id AND user_id = :user_id';
		// $sql = 'INSERT birimdik.favorites WHERE ad_id = :ad_id AND user_id = :user_id';
	    ...
	    if($stmt->rowCount() > 0)
	    {
			...
			return $arr_json = array('status' => 200, 'favorite' => $row);
	    }else{
