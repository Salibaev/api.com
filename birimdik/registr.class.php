...
class Registr
{
	...
	private $db;
	private $method;
	function __construct($login = '', $password = '', $avatar = '',$numbers = '',$name = '',$surname = '',$city = '')
	{
	...
	function doPost(){
	    ...
	    header("Access-Control-Allow-Headers: *");
		//POST method
		$sql = 'INSERT birimdik.users (login,password,avatar,numbers,name,surname,city) VALUES (:login,:password,:avatar,:numbers,:name,:surname,:city)';
	    $stmt = $this->db->prepare($sql);
	    $stmt->bindValue(':login', $this->login);
	    $stmt->bindValue(':password', $this->password);
